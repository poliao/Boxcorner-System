...
public interface RecipeRepository extends JpaRepository<Recipe, String> {
    ...
    @Query("""
                ...
                FROM Recipe r
                WHERE
                  (:jobname IS NULL OR UPPER(r.jobname) LIKE CONCAT('%', UPPER(:jobname), '%'))
                  AND (:jobid IS NULL OR UPPER(r.jobid) LIKE CONCAT('%', UPPER(:jobid), '%'))
                  AND (:recipeid IS NULL OR UPPER(r.recipeid) LIKE CONCAT('%', UPPER(:recipeid), '%'))
            """)
